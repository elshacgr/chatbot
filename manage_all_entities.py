...
class manage_extractor_entities:
    ...
    def extract_entity(self, ori_input_user):
        ...
        if len(df_allArray.index) > 0:
            ...
            df_allArray = df_allArray.sort_values(by=['StartIndex'], ascending=False) # sorting by start index as descending
            
            #Reset Index data frame
            df_allArray = df_allArray.reset_index(drop=True) # reset Index data frame
            print(df_allArray)
            ...
            # print("no entity recognized in string input
            # a = ""
    ...
    # def get_final_value(self, input_user):
        
    #     # self.extract_entity()
    #     final_value = self.extract_entity(input_user)[0]
...
def main():
    ...
    input_string = 'hy "venisa", dua kosong satu, dua nol satu, dua ratus satu'
